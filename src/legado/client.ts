...
import { Script as VmScript } from '../vm-polyfill';
import { DOMParser } from '@xmldom/xmldom';
import * as xpath from 'xpath';

// config removed - using env vars
import {
  BookAcquisitionLink,
  BookCatalogResult,
  BookChapter,
...
const DEFAULT_TIMEOUT_MS = Number(globalThis.LEGADO_TIMEOUT_MS || globalThis.OPDS_TIMEOUT_MS || 20000);
const MAX_TEXT_BYTES = Number(globalThis.LEGADO_MAX_TEXT_BYTES || 3 * 1024 * 1024);
const DEFAULT_LEGADO_SEARCH_PAGES = Number(globalThis.LEGADO_SEARCH_PAGES || 5);
...
/**
 * Apply a chain of .replace() calls to an input string without using new Function().
 * Handles: result.replace(/pattern/flags, "replacement").replace(...)...
 * Returns null if the pattern doesn't match.
 */
function applyReplaceChain(input: string, code: string): string | null {
  // 匹配 .replace(/pattern/[flags], "replacement") 支持转义的 / 和引号
  const replaceRe = /\.replace\(\s*\/((?:[^/\\]|\\.)*)\/([gimuy]*)\s*,\s*"((?:[^"\\]|\\.)*)"\s*\)/g;
  let result = input;
  let matched = false;
  let match: RegExpExecArray | null;
  while ((match = replaceRe.exec(code)) !== null) {
    matched = true;
    try {
      result = result.replace(new RegExp(match[1], match[2] || 'g'), match[3]);
    } catch {
      return null;
    }
  }
  if (!matched) {
    // 匹配 single-quote 版本
    const replaceRe2 = /\.replace\(\s*\/((?:[^/\\]|\\.)*)\/([gimuy]*)\s*,\s*'((?:[^'\\]|\\.)*)'\s*\)/g;
    while ((match = replaceRe2.exec(code)) !== null) {
      matched = true;
      try {
        result = result.replace(new RegExp(match[1], match[2] || 'g'), match[3]);
      } catch {
...
function runJsSnippetRaw(code: string, context: Record<string, any>, timeout = 1000): any {
  ...
  try {
    const script = new VmScript(`(function(){ ${body}\n})()`);
...
; if (typeof res !== 'undefined') return res; return ''; })()`);
  ...
  try {
    const script = new VmScript(`(function(){ return (${body}); })()`);
    const result = script.runInNewContext(sandbox, { timeout });
    if (result !== undefined && result !== null && result !== '') return result;
    if (sandbox.result !== context.result && sandbox.result !== undefined && sandbox.result !== null && sandbox.result !== '') return sandbox.result;
    if (lastPut?.key === 'url') return lastPut.value;
    // Fallback: try direct replace chain evaluation (work in workerd where new Function is blocked)
    const initial = jsonPrimitiveToString(context.result ?? context.src ?? '');
    if (initial) {
      const chainResult = applyReplaceChain(initial, body);
      if (chainResult !== null && chainResult !== initial) return chainResult;
    }
    return '';
  } catch {
    // Fallback: try direct replace chain evaluation
    const initial = jsonPrimitiveToString(context.result ?? context.src ?? '');
    if (initial) {
      const chainResult = applyReplaceChain(initial, body);
      if (chainResult !== null && chainResult !== initial) return chainResult;
...
function readJsonRule(json: any, rule?: string, source?: BookSource, baseUrl?: string): string {
  ...
  const trimmed = rule.trim();
  if (trimmed.includes('{{')) {
    // Handle @js: suffix — do plain text substitution first, then eval JS
    const jsIdx = trimmed.lastIndexOf('@js:');
    if (jsIdx > 0) {
      const tmpl = trimmed.slice(0, jsIdx).trim();
      const jsRule = trimmed.slice(jsIdx).trim();
      const rendered = tmpl.replace(/\{\{(.*?)\}\}/g, (_, expr) => {
        const v = readJsonPath(json, String(expr).trim().replace(/^@json:/, ''));
        return jsonPrimitiveToString(v);
      });
      const jsResult = runJsSnippet(jsRule, { result: rendered, src: rendered, baseUrl, source: source?.legado });
      return jsResult;
    }
    return renderTemplateWithJson(trimmed, json, source as BookSource, baseUrl || sourceBase(source as BookSource));
  }
...
function applyContentJsRule(value: string, jsRule: string): string {
  ...
  if (/window\.comicInfo/.test(value)) {
    try {
      ...
      if (match?.[1]) {
        const comicInfo = new VmScript(`(${match[1]})`).runInNewContext({}, { timeout: 1000 });
...
async function resolveLegadoConfig(username?: string): Promise<ResolvedLegadoConfig> {
  ...
  const envJson = globalThis.LEGADO_SOURCES_JSON;
  if (envJson) {
    try {
  ...
  try {
    const subscriptionSources = await getSubscriptionSources();
...
export class LegadoClient {
  ...
  async getBookDetail(sourceId: string, href: string, fallback?: Partial<BookDetail>): Promise<BookDetail> {
    ...
    if (detailHref && rule.ruleBookInfo) {
      ...
      // 应用 ruleBookInfo.init 设置 JSON 评估上下文（如 init: $.data）
      let jsonCtx = json;
      if (json && !initData && rule.ruleBookInfo?.init) {
        const initRule = rule.ruleBookInfo.init.trim();
        if (!initRule.startsWith('@put:') && !initRule.startsWith(':')) {
          const initVal = readJsonPath(json, initRule);
          if (initVal !== undefined && initVal !== null) jsonCtx = initVal;
        }
      }
      ...
      const read = (itemRule?: string) => initData
        ...
        : json
          ? readJsonRule(jsonCtx, itemRule, source, targetUrl)
      ...
      const chapterCountText = ''
        + jsonPrimitiveToString(readJsonPath(jsonCtx, '$.nums') || readJsonPath(jsonCtx, '$.chapter_nums') || '');
...
declare global {
  ...
  var LEGADO_ENABLED: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_SOURCES_JSON: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_SUBSCRIPTION_URLS: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_TIMEOUT_MS: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_MAX_TEXT_BYTES: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_SEARCH_PAGES: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_CACHE_TTL_MS: string | undefined;
  // eslint-disable-next-line no-var
  var LEGADO_SUBSCRIPTION_MAX_BYTES: string | undefined;
  // eslint-disable-next-line no-var
  var OPDS_ENABLED: string | undefined;
  // eslint-disable-next-line no-var
  var OPDS_TIMEOUT_MS: string | undefined;
  // eslint-disable-next-line no-var
